...
func show_player_container(show:bool):
	if show:
		player.visible = true
	else:
		player.visible = false
		
func enable_player_ui(show:bool):
	if show:
		player.enable_player_ui()
	else:
		player.disable_player_ui()
...
func load_special_level(packed: PackedScene, spawn_from_dir: String = "") -> void:
	if _loading:
		return
	if packed == null:
		push_warning("load_special_level called with null PackedScene")
		return

	_loading = true
	_temp_disable_warp(0.25)  # block re-entrance during and right after
	...
	# Instantiate and add the provided scene
	var level := packed.instantiate()
	if level == null:
		push_warning("Failed to instantiate special level scene")
		_loading = false
		return
	level_container.add_child(level, true)

	# Spawn selection (same naming scheme as your regular levels)
	var spawn_name := "Spawn/Center"
	match spawn_from_dir:
		"North":
			spawn_name = "Spawn/FromSouth"
	...
	if not spawn_point:
		spawn_point = level.get_node_or_null("Spawn/Center")
	if not spawn_point:
		print("Couldn't find any spawns in special level!")
	...
	# Note: We do NOT touch current_cell or emit 'level_changed' here,
	# since this is a special level without a grid cell.
