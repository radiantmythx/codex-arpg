...
var IS_FROZEN_AND_IGNORE: bool = false

var _surface_index_by_name: Dictionary = {}

@export var race_visuals_node : Node3D
@export var hair_visuals_node : Node3D
...
@export var player_race : String
@export var player_height : float = 2.3
...
@export var experience_cap: int = 100
...
@export var skeleton_node:Skeleton3D
...
@export var experience_bar_node_path: NodePath
...
var _movement_skill_start_pos:Vector3
var _movement_skill_max_distance:float
...
func _ready() -> void:
	...
	_surface_index_by_name = _build_surface_name_index(body_mesh)
	_oneshot_timer = Timer.new()
	_oneshot_timer.one_shot = true
	add_child(_oneshot_timer)
...
func _process_movement(delta: float) -> void:
	...
	if _movement_skill_active:
		...
		var world_motion := _movement_skill_velocity * delta
		if _movement_skill_stop_on_collision and not _movement_skill_phase:
			...
			var collision := move_and_collide(local_motion)
			if collision:
				_end_movement_skill()
				return
		else:
		...
		if _movement_skill_timer <= 0.0:
			_end_movement_skill()
		if (global_position.distance_to(_movement_skill_start_pos)) > _movement_skill_max_distance:
			_end_movement_skill()
		return
...
func start_movement_skill(skill: MovementSkill, direction: Vector3, distance: float, dmg_map: Dictionary) -> void:
	...
	_movement_skill_max_distance = skill.max_distance
	_movement_skill_start_pos = global_position
...
func add_experience(expAmount: int, enemyLevel: int = 0) -> void:
	if enemyLevel:
		expAmount *= (1 + (.1 * (enemyLevel - level)))
	if expAmount < 0:
		expAmount = 0
...
func update_exp_bar():
	...
	_experience_bar.max_value = experience_cap
	
func set_frozen_and_ignore(setting: bool):
	IS_FROZEN_AND_IGNORE = setting

func get_skeleton() -> Skeleton3D:
	return skeleton_node
	
func get_race_visuals() -> Node3D:
	return race_visuals_node
	
func get_extra_visuals() -> Node3D:
	return extra_visuals_node
	
func get_hair_visuals() -> Node3D:
	return hair_visuals_node

func reset_race_visuals():
	for c in get_race_visuals().get_children():
		c.queue_free()
		
func reset_extra_visuals():
	for c in get_extra_visuals().get_children():
		c.queue_free()

func reset_hair_visuals():
	for c in get_hair_visuals().get_children():
		c.queue_free()
...
func _build_surface_name_index(mesh_inst: MeshInstance3D) -> Dictionary:
	var map := {}
	...
	for i in range(count):
		var name := ""
		if m.has_method("surface_get_name"):
			name = m.surface_get_name(i)
		if name == "":
...
func set_surface_albedo_and_propagate(
		surface_name: String,
		color: Color,
...
func _dup_tinted_cached(src: Material, color: Color, cache: Dictionary) -> Material:
	...
	if cache.has(id):
		return cache[id]
	var m := src.duplicate(true)
	if m is BaseMaterial3D:
		(m as BaseMaterial3D).albedo_color = color
	elif m is ShaderMaterial:
		(m as ShaderMaterial).set_shader_parameter("albedo_color", color) # adjust param name if needed
	else:
		return null
	cache[id] = m
	return m

func _tint_material(src: Material, color: Color) -> Material:
	return _dup_tinted_cached(src, color, {}) # no sharing needed when only touching self
	
# Plays a state once, then returns to whatever state was active.
# Assumes the target state's animation is non-looping.
func play_state_once_then_return(target_state: String, idle_after: bool = true) -> void:
	_oneshot_gen += 1
	var my_gen := _oneshot_gen

	# Cancel any previous timer wait
	if not _oneshot_timer.is_stopped():
		_oneshot_timer.stop()

	var prev_state := playback.get_current_node()
	if prev_state == "":
		prev_state = target_state

	# Resolve the clip length if this state is a plain AnimationNodeAnimation
	var anim_name := ""
	var node_path := "parameters/StateMachine/%s/node" % target_state
	...
	if anim_name != "" and anim_player.has_animation(anim_name):
		wait_secs = anim_player.get_animation(anim_name).length

	# Start a fresh timer for THIS generation
	_oneshot_timer.start(wait_secs)
	await _oneshot_timer.timeout

	# If a newer request happened while we were waiting, bail out
	if my_gen != _oneshot_gen:
		return
	
	if(!idle_after):
	# Return to the previous state
		playback.travel(prev_state)
	else:
		playback.travel("move")
	


func _wait_for_animation_finished(anim_name: String) -> void:
	# If the animation is already playing through the tree, wait until the player reports it finished.
	# (Ensure the target state’s clip is non-looping.)
	var finished := false
	var sig = anim_player.animation_finished
	while anim_player.is_playing() and anim_player.current_animation == anim_name and not finished:
		var done_anim = await sig
